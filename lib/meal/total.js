...
function recentCarbs(opts, time) {
    console.error("🍔 === MEAL TOTAL DEBUG START ===");
    var treatments = opts.treatments;
    var profile_data = opts.profile;
    if (typeof(opts.glucose) !== 'undefined') {
        var glucose_data = opts.glucose;
    }
    console.error("🍔 Treatments count:", treatments ? treatments.length : 0);
    console.error("🍔 Glucose data points:", glucose_data ? glucose_data.length : 0);
    var carbs = 0;
    var nsCarbs = 0;
    var bwCarbs = 0;
    var journalCarbs = 0;
    ...
    treatments.forEach(function(treatment) {
        ...
        if (treatmentTime > carbWindow && treatmentTime <= now) {
            if (treatment.carbs >= 1) {
                ...
                console.error("🍔 Processing carbs:", treatment.carbs, "at", new Date(treatmentTime));
                var myCarbsAbsorbed = detectCarbAbsorption(COB_inputs).carbsAbsorbed;
                console.error("🍔 CarbsAbsorbed:", myCarbsAbsorbed.toFixed(1)); //?????????????????????????????? here prfile was defined
    ...
    console.error("🍔 Before removal - carbs:", carbs, "toRemove:", carbsToRemove);
    carbs -= carbsToRemove;
    nsCarbs -= nsCarbsToRemove;
    bwCarbs -= bwCarbsToRemove;
    journalCarbs -= journalCarbsToRemove;
    console.error("🍔 After removal - carbs:", carbs, "mealCOB:", mealCOB);
    ...
    console.error("🍔 Calculating deviations...");
    var c = detectCarbAbsorption(COB_inputs);
    console.error("🍔 currentDeviation:", c.currentDeviation ? c.currentDeviation.toFixed(2) : null);
    console.error("🍔 maxDeviation:", c.maxDeviation ? c.maxDeviation.toFixed(2) : null);
    console.error("🍔 minDeviation:", c.minDeviation ? c.minDeviation.toFixed(2) : null);
    ...
    console.error("🍔 === MEAL TOTAL RESULT ===");
    console.error("🍔 Final mealCOB:", Math.round(mealCOB));
    console.error("🍔 Total carbs:", Math.round(carbs * 1000) / 1000);
    console.error("🍔 ============================");
